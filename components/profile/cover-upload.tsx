...
import { Image as ImageIcon, Camera, Trash2, Loader2, AlertCircle } from "lucide-react";
...
export function CoverUpload({
  ...
  return (
    <div className="space-y-2">
      <div className="relative h-36 sm:h-40 w-full rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-gradient-to-r from-heat-500/20 via-amber-500/15 to-heat-600/20 group">
        {coverUrl ? (
          ...
          <div className="h-full w-full flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-500">
            <ImageIcon className="h-7 w-7 mb-1 opacity-50" />
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 p-2">
          <Button
            ...
            className="gap-1.5 text-xs shadow-lg backdrop-blur-md bg-white/90 dark:bg-zinc-900/90"
          >
            {isUploading ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin text-heat-500" />
            ) : (
              <Camera className="h-3.5 w-3.5 text-heat-500" />
            )}
          ...
          {coverUrl && !isUploading && (
            <Button
              ...
              className="gap-1.5 text-xs shadow-lg backdrop-blur-md"
            >
