...
export function ThemeToggle({ className, compact = false }: ThemeToggleProps) {
  ...
  if (!mounted) {
    return (
      <div
        className={cn(
          compact ? "h-9 w-9 rounded-xl bg-zinc-100 dark:bg-zinc-800 animate-pulse" : "h-8 w-full rounded-xl bg-zinc-100 dark:bg-zinc-800 animate-pulse",
          className
        )}
      />
    );
  ...
  // Full segmented mode: 3 distinct buttons in equal widths
  return (
    <div
      className={cn(
        "grid grid-cols-3 gap-1 rounded-xl bg-zinc-200/60 p-1 dark:bg-zinc-900/90 border border-zinc-200/80 dark:border-zinc-800/80",
        className
      )}
      role="group"
      ...
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex items-center justify-center gap-1.5 rounded-lg py-1.5 px-2 text-xs font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-heat-500",
              isActive
